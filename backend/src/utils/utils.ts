...
export class Context {
	...
	downloadsDirAbsolute = 'data_out/downloads'
...
async function downloadPicture(ctx: Context, fullUrl: string, outputDirRelative: string) {
	console.log('downloadPicture', fullUrl)
	const fileDirAbsolute = path.join(ctx.downloadsDirAbsolute, outputDirRelative)
	await mkdir(fileDirAbsolute, { recursive: true })
	const fileBaseName = path.basename(new URL(fullUrl).pathname)
	...
	console.debug(`[DOWNLOAD] ${fullUrl} → ${fileFullPath}`)

	const success = await withRetry(async () => {
    console.debug(`axios.get`, fullUrl)
...
export async function crawlPageForImages(ctx: Context, pageUrl: string, outputDirRelative: string) {
	...
	console.debug(`[DEBUG] Scraping for Pictures...`, pageUrl)

	const imgElements = $('img').toArray() // converts cheerio object to array
    ...
    // const fullSrcUrl = ctx.baseUrl + src
    console.log('image src', src)
    // const fullSrcUrl = 'https://upload.wikimedia.org/' + src // FIXME
    let fullSrcUrl: string
    if ( src.startsWith('//') ) {
      fullSrcUrl = "https:" + src
    } else {
      fullSrcUrl = src.startsWith('https://') ? src : 
      'https://upload.wikimedia.org/' + src // FIXME
    }
    
		downloadPicture(ctx, fullSrcUrl, outputDirRelative)
