...
interface SurrealOdmItemsResponse<TRaw> {
  items: PostgresOdmRow<TRaw>[]
}

/** Mirrors NeonOdmCollectionBackend/MongoOdmCollectionBackend - same HTTP-through-backend shape,
 * since SurrealDB's own driver speaks a WebSocket/RPC protocol tied to a server-held root
 * user/pass, not something safe to hand to a browser, so lifesuite/backend-ts holds the real
...
export class SurrealOdmCollectionBackend<TRaw> extends OdmCollectionBackend<TRaw> {
  ...
  private apiUrl = ((environment as any).surreal?.odmApiUrl ?? (environment.backendUrl ? `${environment.backendUrl}/odm-surreal` : '/api/odm-surreal')).replace(/\/$/, '')
  ...
  private async fetchRows(queryOpts: QueryOpts & {parentId?: ItemId, ancestorId?: ItemId}): Promise<PostgresOdmRow<TRaw>[]> {
    ...
    const response = await this.http
      .get<SurrealOdmItemsResponse<TRaw>>(`${this.apiUrl}/items`, {params})
      .toPromise()
  ...
  private requireUserId(): string {
    ...
    if (!userId) {
      throw this.errorAlertAndThrow('SurrealOdmCollectionBackend before query - no userId')
    }
    return userId
  }

  // When used as a fanout peer, other backends are racing/writing alongside this one, so a
  // Surreal-side failure shouldn't interrupt the user with a window.alert() - just log it.
  private errorAlert(...args: any[]) {
    if (this.opts.silentErrors) {
      console.error('[Surreal ODM]', 'collectionName', this.collectionName, ...args)
      return
    }
